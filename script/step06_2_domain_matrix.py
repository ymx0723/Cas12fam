...
# 更新该函数以直接注释每个SWORD区间
def assign_domains_to_sword_regions(sword_file_path, domain_matrix):
    assigned_lines = []
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python step06_2_domain_matrix.py <dali_file_path> <sword_file_path> <output_file_path>")
        sys.exit(1)
    
    dali_file_path = sys.argv[1]  # 从命令行参数获取DALI注释文件路径
    sword_file_path = sys.argv[2]  # 从命令行参数获取SWORD区域文件路径
    ...
    # 输出分配的结构域信息到文件
    output_file_path = f"{sys.argv[3]}_annotated_sword_result.txt"
    with open(output_file_path, "w") as output_file:
        output_file.write(formatted_result)
    print(f"Annotated SWORD result saved to {output_file_path}")
    
    # 保存矩阵到CSV文件
    domain_matrix.to_csv(f"{sys.argv[3]}_domain_annotation_matrix.csv")