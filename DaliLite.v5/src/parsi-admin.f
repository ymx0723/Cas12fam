...
c
	return
c	error exit
99	write(*,*) 'file not found or dim. error:',filnam
!	if(ldb) then
!		write(*,*) 'type CONTINUE to try again'
!		pause
!	end if
...
c
        ...
        na=0
        nb=0
	read(iunit,500,err=19,end=18) nres,nseg,na,nb,
     $  (secstr(i),i=1,min(maxseg,nseg))
        if(nseg.gt.maxseg) goto 19
	do iseg=1,nseg
	  read(iunit,510,err=19) i,(segmentrange(j,i),j=1,2),
     $ (checkrange(j,i),j=1,2),checkx(i)
	end do
	read(iunit,520) ((ca(j,i),j=1,3),i=1,nres)
	read(iunit,500) ndom
        if(ndom.gt.maxdom) goto 19
	do idom=1,ndom
	  read(iunit,530,err=19) i,node_type(i),(node_child(j,i),j=1,2),
     $		domns(i),(domseglist(j,i),j=1,domns(i))
	end do
